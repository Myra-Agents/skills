...
| [`tauri-ax-test`](./skills/tauri-ax-test) | Authors and runs reusable component UI tests for native macOS apps (Tauri/Electron/WKWebView) by driving the accessibility (AX) tree — no pixel clicks. The agent runs everything; the harness vendors into the app's own repo (`tests/native/`) as a self-locating, indexed test library. |
